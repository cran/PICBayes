...
### main routine ###
   ...
   tau_trt=rgamma(1,a_tau_trt,rate=b_tau_trt)
   gamcoef=matrix(rgamma(K, 1, rate=1),ncol=K)

   phicoef<-matrix(rep(0,I),ncol=1)
   phicoef2<-matrix(rep(NA,N),ncol=1)
   for (j in 1:N){
   ...
   partau_trt=array(rep(0,total),dim=c(total,1))
   ...
   if (is.null(x_user)){parsurv=parsurv0_original} else {
   G<-length(x_user)/p
   ...
   { 
      ...
      for (j in 1:N){
      if (y[j]==0){
      templam1=LambdatR[j]*exp(xcov[j,]%*%beta+phicoef[area[j]]+phicoef_trt[area[j]]*xtrt[j])
      z[j]=poissrndpositive(templam1)
      zz[j,]=rmultinom(1,z[j],gamcoef*t(bisR[,j]))
      } else if (y[j]==1){
      templam1=(LambdatR[j]-LambdatL[j])*exp(xcov[j,]%*%beta+phicoef[area[j]]+phicoef_trt[area[j]]*xtrt[j])
...
beta1[r]<-yt
beta2[r]<-xt
log_f1<-sum(yt*xcov[,r]*te1)-sum(exp(xcov%*%beta1+phicoef2+phicoef2_trt*xtrt)*te2)-0.5*(yt^2)/(beta_sig0^2)
log_f2<-sum(xt*xcov[,r]*te1)-sum(exp(xcov%*%beta2+phicoef2+phicoef2_trt*xtrt)*te2)-0.5*(xt^2)/(beta_sig0^2)
...
if (binary[r]==1 & p>1){
te4=sum(xcov[,r]*te1)
te5=sum(te2*exp(as.matrix(xcov[,-r])%*%as.matrix(beta[-r])+phicoef2+phicoef2_trt*xtrt)*xcov[,r])
beta[r]<-log(rgamma(1,a_ga+te4,rate=b_ga+te5))
}

if (binary[r]==1 & p==1){
te4=sum(xcov[,r]*te1)
te5=sum(te2*exp(phicoef2+phicoef2_trt*xtrt)*xcov[,r])
beta[r]<-log(rgamma(1,a_ga+te4,rate=b_ga+te5))
}
}
...
+(bisL[l,])*(y==2))*exp(xcov%*%beta+phicoef2+phicoef2_trt*xtrt))
...
phi1[area==i]<-yt
phi2[area==i]<-xt
log_f1<-sum(phi1*te1)-sum((exp(xcov%*%beta+phi1+phicoef2_trt*xtrt))*te2)-0.5*tau*(yt^2)
log_f2<-sum(phi2*te1)-sum((exp(xcov%*%beta+phi2+phicoef2_trt*xtrt))*te2)-0.5*tau*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
phi1_trt[area==i]<-yt
phi2_trt[area==i]<-xt
log_f1<-sum(phi1_trt*xtrt*te1)-sum(te2*exp(xcov%*%beta+phicoef2+phi1_trt*xtrt))-0.5*tau_trt*(yt^2)
log_f2<-sum(phi2_trt*xtrt*te1)-sum(te2*exp(xcov%*%beta+phicoef2+phi2_trt*xtrt))-0.5*tau_trt*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
#sample tau_trt
tau_trt<-rgamma(1,0.5*I+a_tau_trt,rate=0.5*t(phicoef_trt)%*%phicoef_trt+b_tau_trt) 

#calculate -2logL 
FL<-1-exp(-LambdatL*exp(xcov%*%beta+phicoef2+phicoef2_trt*xtrt))   # n2*1
FR<-1-exp(-LambdatR*exp(xcov%*%beta+phicoef2+phicoef2_trt*xtrt))   # n2*1 
...
dev<--2*loglike      # -2logL
      ...
      pareta[iter]=eta
partau[iter]=tau
partau_trt[iter]=tau_trt
parphi[iter,]=phicoef
parphi_trt[iter,]=phicoef_trt
      pargam[iter,]=gamcoef
      ttt=gamcoef%*%bisg
	if (scale.designX==FALSE) {parsurv0_original[iter,]<-exp(-ttt)}
      if (scale.designX==TRUE) {parsurv0_original[iter,]<-exp(-ttt*exp(-sum((beta*mean_X/sd_X)[scaled==1])))}
...
if (iter%%100==0) print(iter)
   ...
   FL_m<-1-exp(-LambdatL_m*exp(as.matrix(xcov)%*%as.matrix(beta_m)+phicoef2_m+phicoef2_trt_m*xtrt))
   FR_m<-1-exp(-LambdatR_m*exp(as.matrix(xcov)%*%as.matrix(beta_m)+phicoef2_m+phicoef2_trt_m*xtrt))
