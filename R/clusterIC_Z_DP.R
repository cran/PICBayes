...
### main routine ###
   ...
   LK<-array(rep(NA,I*H),dim=c(I,H))   #used repeatedly for each r=1,...,q

   for (r in 1:q){
   pi[,r]<-rdirichlet(1,rep(alpha[r]/H,H))   #H x q
   }
...
phi1[area==i,r]<-yt
phi2[area==i,r]<-xt
log_f1<-sum(zcov[,r]*phi1[,r]*te1)-sum((exp(xcov%*%beta+apply(zcov*phi1,1,sum)))*te2)-0.5*tau[i,r]*(yt^2)
log_f2<-sum(zcov[,r]*phi2[,r]*te1)-sum((exp(xcov%*%beta+apply(zcov*phi2,1,sum)))*te2)-0.5*tau[i,r]*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
for (i in 1:I){
for (h in 1:H){
LK[i,h]<-dnorm(phicoef[i,r],0,1/sqrt(tau_star[h,r]))   #annealing parameter to improve clustering 
}
...
for (h in 1:H) {
c1=c[,r]
ns[h,r]<-length(c1[c1==h])   #must allow zeros for empty components
}
}
...
#update alpha
for (r in 1:q){
alpha[r]<-rgamma(1,a_alpha+H-1,b_alpha-sum(log(1-v[,r])[1:(H-1)]))   #b_alpha-log(cumv[H-1]) or b_alpha-log(pi[H])
}
