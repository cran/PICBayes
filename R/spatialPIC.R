...
phi1[area==i]<-yt
phi2[area==i]<-xt
log_f1<-sum(phi1[1:n1])-sum(te3*exp(xcov[1:n1,]%*%beta+phi1[1:n1]))+sum(phi1[(n1+1):N]*te1)-sum((exp(xcov[(n1+1):N,]%*%beta+phi1[(n1+1):N]))*te2)-0.5*nn[i]*lamb*(yt-C[i,]%*%phicoef/nn[i])^2
log_f2<-sum(phi2[1:n1])-sum(te3*exp(xcov[1:n1,]%*%beta+phi2[1:n1]))+sum(phi2[(n1+1):N]*te1)-sum((exp(xcov[(n1+1):N,]%*%beta+phi2[(n1+1):N]))*te2)-0.5*nn[i]*lamb*(xt-C[i,]%*%phicoef/nn[i])^2
num<-log_f1 #proposal dist is symmetric
den<-log_f2
