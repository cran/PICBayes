...
### main routine ###
   ...
   L_trt<-array(rep(NA,I*H_trt),dim=c(I,H_trt))
   tau_trt<-rep(NA,I)
...
phi1[area==i]<-yt
phi2[area==i]<-xt
log_f1<-sum(phi1*te1)-sum((exp(xcov%*%beta+phi1+phicoef2_trt*xtrt))*te2)-0.5*tau[i]*(yt^2)
log_f2<-sum(phi2*te1)-sum((exp(xcov%*%beta+phi2+phicoef2_trt*xtrt))*te2)-0.5*tau[i]*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
phi1_trt[area==i]<-yt
phi2_trt[area==i]<-xt
log_f1<-sum(phi1_trt*xtrt*te1)-sum(te2*exp(xcov%*%beta+phicoef2+phi1_trt*xtrt))-0.5*tau_trt[i]*(yt^2)
log_f2<-sum(phi2_trt*xtrt*te1)-sum(te2*exp(xcov%*%beta+phicoef2+phi2_trt*xtrt))-0.5*tau_trt[i]*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
for (i in 1:I){
for (h in 1:H){
L[i,h]<-dnorm(phicoef[i],0,1/sqrt(tau_star[h]))   #annealing parameter to improve clustering 
}
p_c<-(pi*L[i,])/sum(pi*L[i,])
...
for (i in 1:I){
for (h in 1:H_trt){
L_trt[i,h]<-dnorm(phicoef_trt[i],0,1/sqrt(tau_trt_star[h]))   #annealing parameter to improve clustering 
}
p_c_trt<-(pi_trt*L_trt[i,])/sum(pi_trt*L_trt[i,])
