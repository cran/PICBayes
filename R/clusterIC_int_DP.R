...
phi1[area==i]<-yt
phi2[area==i]<-xt
log_f1<-sum(phi1*te1)-sum((exp(xcov%*%beta+phi1))*te2)-0.5*tau[i]*(yt^2)
log_f2<-sum(phi2*te1)-sum((exp(xcov%*%beta+phi2))*te2)-0.5*tau[i]*(xt^2)
num<-log_f1 #proposal dist is symmetric
den<-log_f2
...
#calculate -2logL 
FL<-1-exp(-LambdatL*exp(xcov%*%beta+phicoef2))   # n2*1
FR<-1-exp(-LambdatR*exp(xcov%*%beta+phicoef2))   # n2*1 
